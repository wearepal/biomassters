...
__all__ = ["EMA", "EMACheckpointer", "EMAOptimizer"]
...
class EMA(Callback):
    ...
    def save_original_optimizer_state(self, trainer: "pl.Trainer"):
        def _prepare_optimizers(optimizers: List[Optimizer]) -> TypeGuard[List["EMAOptimizer"]]:
            for optimizer in optimizers:
                if not isinstance(optimizer, EMAOptimizer):
                    return False
                optimizer.save_original_optimizer_state = True
            return True

        assert _prepare_optimizers(trainer.optimizers)
        for optimizer in trainer.optimizers:
            assert isinstance(optimizer, EMAOptimizer)
            optimizer.save_original_optimizer_state = True
        try:
...
class EMAOptimizer(torch.optim.Optimizer):
    ...
    :param optimizer: optimizer to wrap
    :param device: device for EMA parameters
    ...
    def step(self, closure: Optional[LossClosure] = None, **kwargs: Any):
        self.join()
        ...
        loss = self.optimizer.step(closure, **kwargs)  # type: ignore
